...
if($styles['filter'] == '1'){
    ...
    if(count($terms) > 0){
        echo "<ul team-id='" . $this->data->ID ."' class='filter-box' style='
            $layout_align;
            $filter_font_size;
            $filter_bold;
            $filter_italic;
            ...
            '>";
        echo '<li class="isotope-item" data-filter="*">';
        echo '<div class="ats-button ats-button-active">Everyone</div>';
        echo '</li>';
        foreach($terms as $term){
            echo '<li class="isotope-item" data-filter="' . $term->term_id . '">';
            echo '<div class="ats-button">' . $term->name . '</div>';
            echo '</li>';
        }
        echo '</ul>';
    }
