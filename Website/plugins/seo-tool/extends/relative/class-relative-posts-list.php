...
class DX_Seo_Relative_posts_List {
	...
	function do_relative( $content ) {
		...
		$loop = $this->do_loop();
		$setting = get_option( 'dxseo_relative_posts' );
		$title = isset( $setting['title'] ) ? $setting['title'] : '相关内容';
		$title_size = isset( $setting['title-size'] ) ? (int) $setting['title-size'] : 14;
		
		$h_title = '<h5 style="margin:20px 0 5px 0;padding-top:20px;font-weight:bold;font-size:' . $title_size . 'px;;border-bottom: 1px dotted #D3D3D3;width:650px;line-height:30px;">' . $title . '</h5>';
		
		// Output
		if( $loop )
			return $content . '<div id="dxseo-related-posts" style="clear:both;margin:0 0 30px 15px;font-size:12px;">' . $h_title . '<ul class="dxseo-rl-items" style="clear:both;margin:0 0 30px 15px;font-size:12px;">' . $loop . '</ul></div>';
		else
			return $content;
	}
