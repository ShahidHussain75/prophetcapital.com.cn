...
class PostsGrid {
    ...
    public function formatDateHelper( $date ) {
        $t_time = mysql2date( __( 'Y/m/d g:i:s A', 'data-source' ), $date, true );
        $m_time = $date;
        ...
        if ( $time_diff > 0 && $time_diff < 24*60*60 )
            $h_time = sprintf( __( '%s ago', 'data-source' ), human_time_diff( $time ) );
        else
            $h_time = mysql2date( __( 'Y/m/d', 'data-source' ), $m_time );
    ...
    public function orderByHelper( $column, $bootstrap ) {
        ...
        return $bootstrap->menuUrl('dts-'.$bootstrap->currentPage, $bootstrap->currentAction, array('orderby'=>$orderBy, 'order'=>$order));
    }
